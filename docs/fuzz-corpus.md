...
The `fuzz` operation in `.golib.yaml` runs seven bounded native Go fuzz targets.
...
| `adapters/jsonapi/FuzzProjectionPaths` | successor projection parity for arbitrary field/code text, pointer escaping, and JSON encoding | report diagnostic and path limits |
